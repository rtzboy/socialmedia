	...
	return (
		<div ref={refInput} onClick={() => setOpenSearch(true)} className='relative dark:text-white'>
			<input
				...
				placeholder='Search'
				type='text'
				className='w-full max-w-[250px] rounded-lg bg-white px-2 py-1 outline-none dark:bg-black-300'
			/>
			{searchUsers.search &&
				(searchUsers.loading ? (
			...
			>
				{searchUsers.usersData?.length === 0 ? (
					<div className='px-4 pt-1 italic text-slate-600 dark:text-white'>No users found!</div>
				) : null}
